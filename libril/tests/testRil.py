...
class TestRil (unittest.TestCase):
    ...
    def test_ril_at_cmer (self):
        ril.rsp_ok('+CMER: 1,0,0,0,1')
        err, mode, ind, bfr = ril.at_cmer()
        self.assertEqual(err, -Error.NONE)
    ...
    def test_ril_at_cclk (self):
        ril.rsp_ok(f'+CCLK: "14/07/01,15:00:00+01"')
        err, year, month, day, hours, minutes, seconds, timezone = ril.at_cclk()
        self.assertEqual(err, -Error.NONE)
    ...
    def test_ril_at_cmee (self):
        self._at_read('+CMEE', ril.at_cmee, 2)
    ...
    def test_ril_at_cged (self):
        self._at_read('+CGED', ril.at_cged, 3)
    ...
    def test_ril_at_cmgf (self):
        self._at_read('+CMGF', ril.at_cmgf, 1)

    def test_ril_at_cmgf_set (self):
        self._at_set('+CMGF', ril.at_cmgf_set, 1)

    def test_ril_at_csdh (self):
        self._at_read('+CMGF', ril.at_csdh, 0)
